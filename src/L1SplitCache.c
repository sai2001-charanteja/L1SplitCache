 ...
 ************************************************************/
static int ADDRESS_SIZE_BITS= 32;      // 32-bit addresses
static int ILINE_SIZE_BYTES =  64  ;    // 64-byte cache line
static int DLINE_SIZE_BYTES =  64  ;    // 64-byte cache line
static int NUM_SETS_DCACHE  = 16384;   // 16K sets
static int NUM_SETS_ICACHE  = 16384;   // 16K sets
static int IWAYS            = 4    ;   // I$ is 4-way set associative
static int DWAYS            = 8    ;   // D$ is 8-way set associative
...
static int cache_find_invalid_way(cache_t* c, uint32_t index,cache_typee ctype)
{
    ...
    for (int w = 0; w < num_ways; ++w) {
        line_t* ln = &set->ways[w];
        if (ln->mesi == MESI_I && max_way > ln->lru) {
            max_way = ln->lru;
			insert_way = w;
        }
    }
    ...
    if (way >= 0) {
        ...
        switch(ln->mesi) {
            ...
            case MESI_E:
                ...
                break;
            case MESI_M:
                // Modified -> Invalid + write back to L2
                ln->mesi = MESI_I;
                msg_return_data_to_l2();
				break;
        }
...
static void load_cache_config(const char* filename)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, " %127[^=] = %127s", key, value) == 2) {
            if (strcmp(key, "ADDRESS_SIZE_BITS") == 0)
                ADDRESS_SIZE_BITS = atoi(value);
            else if (strcmp(key, "ILINE_SIZE_BYTES") == 0)
                ILINE_SIZE_BYTES = atoi(value);
            else if (strcmp(key, "DLINE_SIZE_BYTES") == 0)
                DLINE_SIZE_BYTES = atoi(value);
            else if (strcmp(key, "NUM_SETS_DCACHE") == 0)
                NUM_SETS_DCACHE = atoi(value);
            else if (strcmp(key, "NUM_SETS_ICACHE") == 0)
                NUM_SETS_ICACHE = atoi(value);
            else if (strcmp(key, "IWAYS") == 0)
                IWAYS = atoi(value);
            else if (strcmp(key, "DWAYS") == 0)
                DWAYS = atoi(value);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    if (argc >= 7 && strcmp(argv[5], "--max-events") == 0) {
        n = atoi(argv[6]);
    }
   ...
   load_cache_config("cache_config.txt");
